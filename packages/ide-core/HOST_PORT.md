...
## 계약 (8 메서드 + 발행 3종, P1 슬라이스4 + 인증 3종, P1 슬라이스5 + 스냅샷 3종, P1 슬라이스6 + 백업 2종, P1 슬라이스7)
...
### `listSnapshots(chapterId) → [{id, label, createdAt, chars}]`

스냅샷/되돌리기(P1 슬라이스6, "글이 안 날아간다" 안전망) — 지정 챕터의 스냅샷을 **최신
순**으로 나열한다. `html` 은 싣지 않는다(목록 경량, `listChapters()`와 같은 이유) — 대신
`chars`(태그를 벗긴 순수 텍스트 길이, 정확한 워드카운트가 아니라 "분량이 얼마나
됐었나"를 가늠하는 가벼운 지표)로 스냅샷 간 분량을 비교할 수 있게 한다. `label` 이
`null` 이면 자동 스냅샷(아래 저장/복원 규칙 참고), 문자열이면 "지금 스냅샷"으로 사용자가
직접 붙인 라벨이다.
...
// 응답
[
  { "id": 12, "label": "탈고 직전", "createdAt": "2026-07-08T11:20:00", "chars": 842 },
  { "id": 9, "label": null, "createdAt": "2026-07-08T10:50:00", "chars": 810 }
]
```

**자동 스냅샷 규칙(두 가지, 둘 다 `label: null`)**:
- `saveChapter()` 처리 시 — 해당 챕터의 최신 스냅샷이 10분보다 오래됐으면(또는 아예
  없으면) *저장으로 덮어써지기 직전의 현재 상태*를 자동 스냅샷한다("세션 단위 안전망").
- `restoreSnapshot()` 처리 시 — *복원으로 덮어써지기 직전의 현재 상태*를
  `label: "복원 전 자동"` 로 자동 스냅샷한다(되돌리기를 되돌릴 수 있게).

**상한** — 챕터당 30개 초과 시 오래된 것부터 prune(라벨 유무 무관, 단순성 우선). 다른
챕터의 스냅샷 개수와는 무관하게 챕터별 독립 카운트.

### `takeSnapshot(chapterId, label?) → {id}`

"지금 스냅샷" 버튼 — 호출 시점 챕터의 현재 상태(title/html)를 즉시 스냅샷한다. **자동
스냅샷의 10분 스로틀과 무관하게** 매 호출마다 찍힌다(사용자가 명시적으로 요청한 것).
`label` 을 생략하면(또는 빈 문자열) `null` 로 저장돼 목록에서 "자동 저장"처럼 보이므로,
UI(`prompt()`)에서 빈 입력을 명시적 `null` 로 정규화해 넘긴다.

```json
// 요청
{ "chapterId": 1, "label": "탈고 직전" }
// 응답
{ "id": 12 }
```

### `restoreSnapshot(snapshotId) → {id, title, synopsis, status, html}`

스냅샷 시점의 **html 만** 되돌린다(title/synopsis/status 는 그대로 — 안전망의 대상은
본문이다). 응답은 `loadChapter()`와 같은 형태의 **복원된 챕터 전체**라 호출 측이 그대로
에디터에 다시 넣으면 된다(에디터 리로드용). 복원 전 자동 스냅샷 규칙(위 참고)이 먼저
실행된 뒤 덮어쓴다. 원본 챕터가 이미 삭제된 고아 스냅샷이면(복원할 대상이 없음) 에러.

```json
// 요청
{ "snapshotId": 12 }
// 응답
{ "id": 1, "title": "1장", "synopsis": "발단", "status": "DRAFT", "html": "<article data-juldoc=\"1\"><p>탈고 직전 본문</p></article>" }
```

### `backupNow() → {savedCount, skippedCount, backedUpAt}`

원고 백업(P1 슬라이스7, `desktop/backup.py` + 서버 `ms.manuscript_book`/`manuscript_revision`
— 백엔드 `backend/src/features/manuscript/`) — 로컬 책의 **모든 챕터**를 서버로 일방향
push한다(양방향 동기화는 P2, 지금은 백업뿐 — 서버 상태를 로컬로 되읽어오지 않는다).
`book.sync_key`(데스크탑이 최초 오픈 시 생성하는 영구 식별자)로 서버 책을 식별 — 서버에
그 sync_key 책이 없으면 새로 만들고, 있으면 그 책에 이어서 리비전을 쌓는다.

챕터별로 서버가 직전 리비전과 내용(`content_hash` = sha256(html))을 비교해 같으면
`skippedCount`에, 다르면 새 리비전으로 저장하고 `savedCount`에 센다 — 자주 눌러도(또는
자동 백업이 반복돼도) 내용 변화가 없으면 리비전이 쌓이지 않는다. 실패(네트워크 오류·
미로그인 401 등)는 에러로 던진다(조용한 실패 없음 — 사용자가 명시적으로 누른 동작).

```json
// 응답
{ "savedCount": 2, "skippedCount": 1, "backedUpAt": "2026-07-08T15:20:00" }
```

**자동 백업** — `saveChapter()` 처리 후, 토큰이 있고 마지막 *성공한* 백업이 15분보다
오래됐으면(또는 아예 없으면) 호스트가 백그라운드로 best-effort push를 시도한다. 실패해도
타이핑·저장에 전혀 영향 없다(조용히 삼킴, 결과는 다음 `getBackupStatus()` 조회에만
반영) — "로컬우선" 원칙: 백업은 안전망이지 저장의 전제조건이 아니다.

### `getBackupStatus() → {lastBackupAt: string|null}`

상단바 "마지막 백업 시각" 표시용 — 네트워크 호출 없이 로컬에 기록된 마지막 **성공한**
백업 시각만 읽는다(수동 `backupNow()` 든 자동 백업이든 성공하면 갱신됨). 백업한 적이
없으면 `null`.

```json
// 응답
{ "lastBackupAt": "2026-07-08T15:20:00" }
// 응답(백업한 적 없음)
{ "lastBackupAt": null }
```
...
- **Python(`desktop/app.py`/`store.py`, 구현됨)**: `js_api` 메서드명은 Python 관례상
  ...
  `save_settings`, `publish`, `login`, `logout`, `whoami`, `list_snapshots`,
  `take_snapshot`, `restore_snapshot`, `backup_now`, `get_backup_status`) — `host.js` 의
  pywebview 어댑터가 이를 camelCase 계약으로 매핑한다. 반환 dict 의 필드명 자체는 이미 이
  문서의 camelCase 규칙을 따른다(`status_cd` 컬럼 → `"status"` 키). 로그인 토큰의 정본
  저장소는 `desktop/token_store.py`(OS 키체인) — `store.py`의 평문 `setting.token`은 dev
  수동 입력 전용 폴백이다. 백업 로직(`push_backup`)은 `desktop/backup.py`, 원격 push는
  `publisher.py`의 `_request()`를 재사용한다.
