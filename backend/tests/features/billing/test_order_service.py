...
async def test_confirm_creates_settlement_with_server_price():
    ...
    oid = await svc.create_order(uuid.uuid4(), uuid.uuid4(), "SELF")
...
async def test_external_channel_uses_60_percent():
    ...
    oid = await svc.create_order(uuid.uuid4(), uuid.uuid4(), "EXTERNAL")
