...
class TestOwnedDocumentLockdown:
    async def test_owner_can_save_others_cannot(self, override_db, tokens):
    ...
    async def test_owner_can_delete_others_cannot(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "A's doc", _auth(tokens["A"]))
    ...
    async def test_owner_can_export_others_cannot(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "A's doc", _auth(tokens["A"]))
    ...
    async def test_owner_can_share_others_cannot(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "A's doc", _auth(tokens["A"]))
    ...
    async def test_owner_can_revoke_others_cannot(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "A's doc", _auth(tokens["A"]))
    ...
    async def test_owner_can_list_shares_others_cannot(self, override_db, tokens):
...
class TestOwnerlessUnchanged:
    async def test_anon_created_doc_editable_by_anyone(self, override_db, tokens):
        async with await _client() as c:
    ...
    async def test_anon_can_list_shares_of_ownerless_doc(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "public doc")
    ...
    async def test_upload_while_anon_is_ownerless(self, override_db, tokens):
        async with await _client() as c:
            r = await c.post(
...
class TestListVisibility:
    async def test_login_sees_own_plus_ownerless_others_hidden(self, override_db, tokens):
        async with await _client() as c:
    ...
    async def test_anon_sees_only_ownerless(self, override_db, tokens):
        async with await _client() as c:
            a_doc = await _create(c, "A doc", _auth(tokens["A"]))
            public = await _create(c, "public doc")
...
class TestMineFlag:
    async def test_mine_true_for_owner_false_for_ownerless(self, override_db, tokens):
        async with await _client() as c:
    ...
    async def test_mine_false_for_other_viewer(self, override_db, tokens):
        async with await _client() as c:
            owned = await _create(c, "owned", _auth(tokens["A"]))
...
class TestSharePublicAccessIgnoresOwnership:
    async def test_share_token_view_works_without_auth(self, override_db, tokens):
        async with await _client() as c:
    ...
    async def test_edit_share_saves_without_auth_for_owned_doc(self, override_db, tokens):
        async with await _client() as c:
    ...
    async def test_export_share_downloads_without_auth_for_owned_doc(self, override_db, tokens):
        async with await _client() as c:
            doc_id = await _create(c, "A doc", _auth(tokens["A"]))
