...
def app_db(sessionmaker):
    """3-provider(작가·리뷰어·타인) 동시 로그인이 필요 — conftest의 단일 social_profile app_db로 못 대체."""
    async def _session():
...
async def test_campaign_full_flow(client_orders):
    c = client_orders
    ...
    book = await create_book(c, a_auth, title="서평캠페인책")
...
async def test_open_campaigns_filter_by_category(client_orders):
    ...
    async def make(title, category):
        bid = await create_book(c, a_auth, title=title)
...
async def test_author_can_close_campaign(client_orders):
    ...
    book = await create_book(c, a_auth, title="마감책")
...
async def test_cancel_application(client_orders):
    c = client_orders
    author_token, _ = await login_account(c, "google", "a")
    reader_token, reader = await login_account(c, "naver", "r")
    a_auth = {"Authorization": f"Bearer {author_token}"}
    ...
    book = await create_book(c, a_auth, title="취소책")
    await c.put(f"/api/books/{book}/price", json={"amount": 1000}, headers=a_auth)
    await c.post(f"/api/books/{book}/publish-now", headers=a_auth)
    cid = (await c.post("/api/campaigns", json={"bookId": book, "slots": 1, "withdrawalConsent": True}, headers=a_auth)).json()["campaignId"]
...
async def test_assign_notifies_reviewer(client_orders):
    ...
    book = await create_book(c, a_auth, title="알림책")
    await c.put(f"/api/books/{book}/price", json={"amount": 1000}, headers=a_auth)
    await c.post(f"/api/books/{book}/publish-now", headers=a_auth)
    cid = (await c.post("/api/campaigns", json={"bookId": book, "slots": 1, "withdrawalConsent": True}, headers=a_auth)).json()["campaignId"]
    await c.post(f"/api/campaigns/{cid}/apply", headers=r_auth)
...
async def test_assign_only_by_campaign_author(client_orders):
    c = client_orders
    author_token, _ = await login_account(c, "google", "a")
    reader_token, reader = await login_account(c, "naver", "r")
    other_token, _ = await login_account(c, "kakao", "o")
    ...
    book = await create_book(c, a_auth, title="캠책2")
    await c.put(f"/api/books/{book}/price", json={"amount": 1000}, headers=a_auth)
    await c.post(f"/api/books/{book}/publish-now", headers=a_auth)
    cid = (await c.post("/api/campaigns", json={"bookId": book, "slots": 1, "withdrawalConsent": True}, headers=a_auth)).json()["campaignId"]
    await c.post(f"/api/campaigns/{cid}/apply", headers=r_auth)
