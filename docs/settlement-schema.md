...
>
> ✅ **구현 반영(2026-07-09)** — 아래는 초기 설계이고, 실제 구현·결정은:
> - **자체 출금플로우**(토스 정산대행 안 씀 — 정산대행=타인대금 매개로 전자금융업 선 넘음, lr-d9e3ed3c). 전자금융업 회피는 **구조**(판매자·royalty)에서 나오지 승인/이체 방식 아님(lr-a0a8bda9).
> - **환불세이프 게이트**(lr-71579146): 출금가능액=오픈(`book_order.delivered_ts`) OR 결제+7일 지난 정산만. 근거=청약철회 일반 7일(§17① / 하자·표시상이는 §17③ 3개월내 안날30일, 별개), lr-6b6f01e3.
> - **정산 주기=매주 수요일 고정 배치**(M+2 아님) — 그 수요일까지 환불세이프 윈도우에 든 정산분 지급. `bill.settlement_run`(run_dt UNIQUE 멱등) + 스케줄러. periodic royalty라 실질=판매자 방어.
> - **미구현/후속**: ledger 원장·§17③ 출금후환불 자동상계(현재 로그+수동)·자동승인·실이체 자동화.
>
> 현재: `bill.book_order`(구매) + `bill.settlement`(주문당 작가몫 계산, 원천징수 3.3% 계산 이미 있음) + `bill.payout`(REQUESTED→APPROVED→PAID) + `bill.bank_account` + `bill.settlement_run`(주간배치 가드) 구현됨.
